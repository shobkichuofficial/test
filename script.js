  ...
  <div class="card-modern group cursor-pointer overflow-hidden animate-fade-in" data-product-id="${p.id}">
    <div class="p-0">
      <div class="relative overflow-hidden" data-action="preview">
        ...
        ${p.freeDelivery ? `<div class="absolute top-4 right-4 h-20 w-20 bg-red-500 rounded-full flex items-center justify-center text-white font-bold text-center leading-tight shadow-lg animate-pulse z-10"><span class="text-lg">ফ্রি<br>ডেলিভারি</span></div>` : ''}
...
function renderOffers() {
    const offerProducts = PRODUCTS.filter(p => p.freeDelivery).slice(0, 3);
    offerGrid.innerHTML = offerProducts.map(p => `
    <div class="card-modern group cursor-pointer overflow-hidden animate-fade-in border-2 border-destructive/50" data-product-id="${p.id}">
        <div class="p-0">
            <div class="relative overflow-hidden" data-action="preview">
                <img src="${p.image}" alt="${p.name}" class="w-full aspect-square object-cover transition-transform duration-500 group-hover:scale-110" />
            ...
            <div class="p-6 text-center">
                ...
                <div class="flex items-center justify-center gap-4 my-3">
                    <span class="bg-gradient-secondary text-secondary-foreground font-bold text-xl px-4 py-1 rounded-md">৳${p.price}</span>
                    <span class="text-md text-muted-foreground line-through">৳${Math.round(p.price * 1.15)}</span>
                </div>
...
async function handleFormSubmission(form, modal, type) {
    ...
    const totalAmount = parseFloat(form.querySelector('#total-amount').textContent.replace('৳', ''));
    let itemsData, isFreeDelivery;
    if (type === 'single') {
        ...
        isFreeDelivery = CART.length > 0 && CART.every(cartItem => PRODUCTS.find(p => p.id === cartItem.id)?.freeDelivery);
    }
    const fullAddress = `${form.address.value}, থানা: ${form.thana.value}, জেলা: ${form.district.value}, বিভাগ: ${form.division.value}`;
    let deliveryLocationText = isFreeDelivery ? 'Free Delivery Offer' : (form.district.value === 'ঢাকা' ? 'Inside Dhaka (70)' : 'Outside Dhaka (130)');
    const success = await submitToGoogleSheet({ customerName: form.customerName.value, phoneNumber: form.phoneNumber.value, address: fullAddress, deliveryLocation: deliveryLocationText, items: itemsData, totalAmount: totalAmount });
    if (success) {
        closeModal(modal);
        openModal(getThankYouModalHTML());
        setTimeout(() => closeModal(modalsContainer.lastElementChild), 3000);
        if (type === 'cart') { CART = []; updateCartCount(); }
    }
...
modalsContainer.addEventListener('change', e => {
    ...
    if (e.target.name === 'division') {
        ...
        if (division && DIVISIONS_AND_DISTRICTS[division]) {
            DIVISIONS_AND_DISTRICTS[division].forEach(district => { districtSelect.innerHTML += `<option value="${district}">${district}</option>`; });
            districtSelect.disabled = false;
        } else { districtSelect.disabled = true; }
        districtSelect.value = ''; thanaSelect.value = '';
    ...
    if (e.target.name === 'district') {
        ...
        if (district && DISTRICTS_AND_THANAS[district]) {
            DISTRICTS_AND_THANAS[district].forEach(thana => { thanaSelect.innerHTML += `<option value="${thana}">${thana}</option>`; });
            thanaSelect.disabled = false;
        } else { thanaSelect.disabled = true; }
        thanaSelect.value = '';
        updatePriceSummary(form, formType);
    }
});
...
modalsContainer.addEventListener('click', async e => {
  ...
  if (form && e.target.type === 'submit') {
      e.preventDefault();
      if (form.id === 'cart-order-form') await handleFormSubmission(form, modal, 'cart');
      else if (form.id === 'single-order-form') await handleFormSubmission(form, modal, 'single');
  }
  if (action?.includes('single-qty')) {
    const qtyEl = modal.querySelector('#quantity-value');
    let qty = parseInt(qtyEl.textContent, 10);
    if (action === 'inc-single-qty') qty++; else if (action === 'dec-single-qty' && qty > 1) qty--;
    qtyEl.textContent = qty;
    modal.querySelector('#single-qty-dec-btn').disabled = (qty <= 1);
    updatePriceSummary(form, 'single');
  } else if (action?.includes('qty')) {
    const itemEl = e.target.closest('[data-item-id]'); if (!itemEl) return;
    const itemId = itemEl.dataset.itemId;
    const item = CART.find(i => i.id === itemId); if (!item) return;
    if (action === 'inc-qty') item.quantity++; else if (action === 'dec-qty' && item.quantity > 1) item.quantity--; else if (action === 'dec-qty' && item.quantity === 1) { CART = CART.filter(i => i.id !== itemId); itemEl.remove(); }
    if (item) {
...
function updatePriceSummary(form, type) {
    ...
    if (type === 'single') {
        ...
        if (SELECTED_PRODUCT.freeDelivery) isFreeDelivery = true;
    } else {
        subtotal = CART.reduce((sum, item) => sum + item.price * item.quantity, 0);
        form.querySelector('#cart-subtotal').textContent = `৳${subtotal}`;
        if (CART.length > 0 && CART.every(cartItem => PRODUCTS.find(p => p.id === cartItem.id)?.freeDelivery)) isFreeDelivery = true;
    }
    if (selectedDistrict && !isFreeDelivery) deliveryCharge = selectedDistrict === 'ঢাকা' ? 70 : 130;
    const total = subtotal + deliveryCharge;
    const deliveryChargeEl = form.querySelector('#delivery-charge');
    if (isFreeDelivery) { deliveryChargeEl.textContent = 'ফ্রি'; deliveryChargeEl.classList.add('text-success', 'font-bold'); }
    else { deliveryChargeEl.textContent = `৳${deliveryCharge}`; deliveryChargeEl.classList.remove('text-success', 'font-bold'); }
