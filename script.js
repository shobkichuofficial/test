...
function renderProducts(productsToRender) {
  const regularProducts = productsToRender.filter(p => !p.freeDelivery);
  if (regularProducts.length === 0 && productsToRender.length > 0) {
    productGrid.innerHTML = `<div class="col-span-full text-center py-16"><p class="text-muted-foreground mt-2">অন্যান্য সকল পণ্য অফার সেকশনে রয়েছে।</p></div>`;
    ...
    productGrid.innerHTML = regularProducts.map(p => createProductCardHTML(p)).join('');
  }
}

function renderOffers(productsToRender) {
    const offerProducts = productsToRender.filter(p => p.freeDelivery);
    if (offerProducts.length === 0) {
        document.getElementById('offers').style.display = 'none';
    } else {
        document.getElementById('offers').style.display = 'block';
        offerGrid.innerHTML = offerProducts.map(p => createOfferCardHTML(p)).join('');
    }
}
...
function handleSearch(event) {
    ...
    const filteredProducts = PRODUCTS.filter(product => {
        ...
        const productWeight = normalizeSearchTerm(product.weight);
        const productDescription = product.description.toLowerCase();

        return productName.includes(searchTerm) ||
               productNameEn.includes(searchTerm) ||
               productDescription.includes(searchTerm) ||
               productWeight.includes(normalizedSearch);
    });
